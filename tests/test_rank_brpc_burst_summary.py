...
class RankBRPCBurstSummaryTest(unittest.TestCase):
    ...
    def test_post_rank_host_network_rollout_does_not_overlap_ports(self):
        ...
        self.assertIn("wait_ports_free", script)
        self.assertIn("/proc/net/tcp", script)
        self.assertIn("scale deployment/\"$deployment\" --replicas=0", script)
        self.assertIn("timed out deleting old hostNetwork pods", script)
        self.assertIn("timed out waiting for $role hostNetwork ports", script)
        self.assertIn("failed to query old hostNetwork pods", script)
        self.assertIn("diagnose_post_rank_hop_startup_failure.sh", script)
        self.assertIn("Diagnose failed $role rollout", script)
        self.assertIn("pairec_post_rank_binary_identity", script)
        self.assertNotIn("rollout restart deployment/post-rank-hop", script)
        self.assertLess(script.index("wait_pods_gone"), script.index("wait_ports_free"))

    def test_post_rank_business_lanes_measure_real_latency_without_padding(self):
        ...
        self.assertIn("post-rank pressure worker thread creation failed: lane=", inner)
        self.assertIn("first_failure=", inner)
        self.assertIn("health_rpc code=", inner)
        self.assertIn("catch (const std::system_error& exception)", inner)
        self.assertNotIn("MinimumLocalWindow", outer)
        self.assertNotIn("time.Sleep(hold)", coordinator)
        self.assertNotIn("business_hold_ms", coordinator)
        self.assertNotIn("minimum_local_window_ms", inner)
        self.assertNotIn("requested_hold_ms", inner)
        self.assertNotIn("sleep_for(std::chrono::microseconds", inner)
    ...
    def test_post_rank_benchmark_has_latency_and_pressure_gates(self):
        ...
        self.assertIn("BUILD_HOP_IMAGE=0 requires the exact local image", benchmark)
        self.assertIn("set both HOP_BUILD_IMAGE and SOURCE_COMMIT", benchmark)
        self.assertIn('"PAIREC_POST_RANK_TWO_HOP_B_N3_OK"', benchmark)
        self.assertIn('post_rank_hop1_front_brpc_ms', benchmark)
        self.assertIn('post_rank_hop2_front_brpc_ms', benchmark)
        self.assertNotIn('local_business_window_ms', benchmark)
        self.assertNotIn('business_hold_ms', benchmark)
    ...
    def test_post_rank_ub_changes_only_hop1_to_hop2_transport(self):
        ...
        hop_runner = (ROOT / "scripts" / "run_brpc_ub_post_rank_hop.sh").read_text()
        ...
        self.assertIn('HOP2_HOST="${HOP2_HOST:-127.0.0.1}"', hop_runner)
