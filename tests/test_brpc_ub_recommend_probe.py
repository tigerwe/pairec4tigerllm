...
class BrpcUbRecommendProbeTest(unittest.TestCase):
    ...
    def test_known_good_builder_freezes_827_boringssl_baseline(self):
        ...
        self.assertIn("BRPC_KNOWN_GOOD_BINARY_OK", script)
        self.assertIn("boring_ssl_config=explicit_define", script)
        self.assertNotIn("does not contain BoringSSL build evidence", script)
        self.assertIn("//example:echo_c++_server", script)
        self.assertIn("//example:echo_c++_client", script)
        self.assertIn("BRPC_KNOWN_GOOD_ECHO_BUILD_OK", script)
