...
> 最新补充(7/3-真实链路 MGet): 明确当前任务不是旁路 `datasystem_kv_probe`，而是在真实 `PaiRec -> brpc -> inference-brpc-trtllm -> backend=trtllm_cpp` 推荐链路内构造 DataSystem 批量写读。`brpc_inference_server` 新增 `--trt_datasystem_mget_probe=1`，默认关闭；开启后 `trtllm_cpp` 后端每次 Recommend 在完成 TRT runner 和 item mapping 后执行 `MCreate(4) + MSet(4 buffers) + Get(vector<4 keys>)`，日志输出 `method=TrtllmDatasystemMSetMGet object_count=4 set_buffer_count=4 get_key_count=4 found=4 backend=trtllm_cpp`，并将 `kv_write_ms/kv_lookup_ms/kv_source=trtllm_cpp_datasystem_mset_mget` 写入 response trace。两个 cross-node manifest 已开启该开关，`scripts/trace_single_brpc_datasystem_request.sh` 会在 summary 中展示 `trt_datasystem_mget_probe_events`。下一步远端重建/导入 brpc TRT-LLM inference 镜像，apply cross-node manifest，使用 `SIZE=1` 与具体 `BRPC_TARGET=pod/inference-brpc-trtllm-...` 采一次真实链路证据。
...
| **7/3** | **真实推荐链路内 DataSystem MSet/MGet 构造能力落地。`cpp/brpc_gateway/brpc_inference_server.cpp` 为 `backend=trtllm_cpp` 新增 `--trt_datasystem_mget_probe=1`，开启后同一次真实 Recommend 会先走 TRT-LLM C++ runner 和 semantic map，再构造 4 个 `3670016B` 对象执行 `MCreate + MSet(buffers) + Get(vector)`；日志字段包含 `object_count/set_buffer_count/get_key_count/found`，用于证明一次请求内 4 个写对象与 4 个读 key 均命中。两个 cross-node manifest 已显式开启该开关；`scripts/trace_single_brpc_datasystem_request.sh` 新增 `trt_datasystem_mget_probe_events` 汇总。验证：trace 脚本 `bash -n`、cross-node YAML parse、`git diff --check` 通过；真实 C++ 编译和远端 GPU/SDK runtime 验证待重建镜像后执行。** |
