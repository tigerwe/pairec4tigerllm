...
> 最后更新: 2026-06-24 | 当前状态: F14 PaiRec Go brpc client 端到端验证通过；DataSystem worker 池化第一版已落地到本仓：新增共享 ETCD + worker DaemonSet 清单、ServiceDiscovery smoke、TensorRT-LLM `KVClient` 从固定 host/port 切到可选 `DATASYSTEM_ETCD_ADDRESS` 的 patch，并已给 `inference-brpc-trtllm` 补齐 ETCD/亲和环境变量；当前尚未在远端重建带 patch 的 TensorRT-LLM C++ runtime，因此运行中的 TRT-LLM 进程仍可能是旧固定 endpoint 逻辑 | 下一步: 远端先确认 master/worker1 均有 `zcx-pairec-image:v1.1`，apply DataSystem pool 并跑 ServiceDiscovery Set/Get smoke；随后把 patch 编进 TensorRT-LLM C++ runtime/base image，重建 `pairec-brpc-inference:k8s-arm64-trtllm-v1` 后确认日志出现 `with ServiceDiscovery`；RH2D 仍需单独确认 Ascend NPU/CANN/RoCE 环境与 `MSetD2H/MGetH2D` 设备指针接入方案
