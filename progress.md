...
> 最后更新: 2026-06-23 | 当前状态: F14 PaiRec Go brpc client 第一版已接入：`TRTLLMClient` 支持 `protocol=http|brpc`，新增 vendored/offline 友好的 baidu_std brpc/TCP client，主路径可直连 `inference-brpc-trtllm:18100`，并保留 HTTP fallback；已修 PaiRec 小镜像构建不再依赖 Alpine `apk add` 联网安装 `bash/curl`；本机 `go test -mod=vendor ./services/...` 与 `go build -mod=vendor ./...` 通过 | 下一步: 远端 pull 后重建/替换 PaiRec server 镜像，应用 `k8s/configmap-brpc.yaml`，用 PaiRec `/api/recommend` 验证 Go -> brpc/TCP -> C++ TRT-LLM 链路
