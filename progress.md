> 2026-08-12 F19 回归根因锁定到当前TRT库及其非Release构建：三模式对照为历史pair=`32 token/94.75ms`、历史gateway+当前TRT=`32 token/188.75ms`、当前pair=`31 token/176ms`，只替换当前`libtensorrt_llm.so`即增加`94ms`，排除gateway/token数/归因开关。worker1 `CMakeCache.txt`确认`CMAKE_BUILD_TYPE:STRING=`为空，因此`CMAKE_*_FLAGS_RELEASE=-O3 -DNDEBUG`未生效。首次Release重配又暴露构建容器库污染：整目录挂载宿主`/lib64`并加入`LD_LIBRARY_PATH`，使旧`libstdc++`覆盖容器版本，PyTorch缺`GLIBCXX_3.4.32`。构建脚本现仅挂载`libcuda.so.1`单文件，不再把`/host-driver`加入搜索路径，同时强制并校验Release。下一步worker1重编、部署并重跑三模式对照，验收目标为当前暖态恢复到历史约95ms。F19保持in_progress。
>
