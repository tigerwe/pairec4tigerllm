> 2026-08-11 F19 真实 DataSystem I/O 请求级归因通过：严格 runtime Health 正常；连续3轮均执行 inference reset、195请求 prime 和单次 PaiRec replay，`valid=3/3`。每轮旧 KVC 窗口证据均为3次offload+2次onboard，唯一同 UUID native completion 也均为`set_count=3/get_count=2`、`attribution_complete=true`，失败/pending/unknown为0，Pod无重启和crash。三轮E2E=`246.302/258.400/264.759ms`，均值`256.487ms`；BRPC差值均为`4ms`；KVC均值`33.714ms`（offload `19.881ms`、onboard `13.833ms`）。1000请求严格验收在正式循环前连续暴露三处脚本门禁问题：输出目录顺序、滚动更新时`.items[0]`不确定Pod选择，以及把可能被大量DEBUG日志轮转的早期`datasystem_attribution_ready`当作长期证据。第三次日志已在当前Ready Pod中明确看到warmup UUID `0f5b4fc5-...` 的completion（correlation=199、Set=2、complete=true），证明native能力正常而marker证据失效。现以warmup开始时间后的唯一同UUID completion替代marker，并严格检查complete、失败/pending/unknown；正式1000请求在发送前同时启动PaiRec/inference `kubectl logs -f`流式落盘，等待全部UUID闭合后再停止，避免早期trace被DEBUG日志轮转。异常退出自动清理collector；F19相关25项测试通过。下一步拉取后原命令重跑1000请求，再做相同缓存形态三组disabled/enabled A/B。F19保持in_progress。
>
