...
| 日期 | 进度 |
|------|------|
| **6/25** | **补充并修复 DataSystem pool smoke 卡顿诊断脚本：远端 `datasystem-pool-etcd` 与两个 `datasystem-pool-worker` 已进入 Running，但 `scripts/test_datasystem_pool_smoke.sh` 执行无输出，后续诊断确认根因是 `kubectl exec ... python -` 缺少 `-i`，导致 heredoc 没有传入容器，Python 实际没有执行脚本内容。新增 `scripts/debug_datasystem_pool_smoke.sh`，每段都有明确标题和 `KUBECTL_TIMEOUT` 外层超时，支持通过 `POD=` 指定 worker Pod；同时修复 `scripts/test_datasystem_pool_smoke.sh` 与 debug 脚本中的 Python heredoc exec 为 `kubectl exec -i`，并兼容用户误写的 `EXPECT_MIN_WORKKERS`。验证：`bash -n scripts/test_datasystem_pool_smoke.sh scripts/debug_datasystem_pool_smoke.sh` 与 `git diff --check` 通过。** |
