...
> 最后更新: 2026-06-25 | 当前状态: F14 PaiRec Go brpc client 端到端验证通过；DataSystem pool 远端 Pod 已达到 Running，但 `test_datasystem_pool_smoke.sh` 出现无输出卡顿，已新增分段诊断脚本定位 `kubectl exec`、Python DataSystem import、ETCD 连接、ServiceDiscovery 和 KV set/get 卡点；当前尚未在远端重建带 ServiceDiscovery patch 的 TensorRT-LLM C++ runtime，因此运行中的 TRT-LLM 进程仍可能是旧固定 endpoint 逻辑 | 下一步: 远端执行 `EXPECT_MIN_WORKERS=2 bash scripts/debug_datasystem_pool_smoke.sh` 找出 smoke 卡点；通过后再把 patch 编进 TensorRT-LLM C++ runtime/base image，重建 `pairec-brpc-inference:k8s-arm64-trtllm-v1` 并确认日志出现 `with ServiceDiscovery`；RH2D 仍需单独确认 Ascend NPU/CANN/RoCE 环境与 `MSetD2H/MGetH2D` 设备指针接入方案

## 时间线

| 日期 | 进度 |
|------|------|
| **6/25** | **补充 DataSystem pool smoke 卡顿诊断脚本：远端 `datasystem-pool-etcd` 与两个 `datasystem-pool-worker` 已进入 Running，但 `scripts/test_datasystem_pool_smoke.sh` 执行无输出，判断需要把 `kubectl exec`、Python import、ETCD 连接、ServiceDiscovery 和 KV set/get 分段定位。新增 `scripts/debug_datasystem_pool_smoke.sh`，每段都有明确标题和 `KUBECTL_TIMEOUT` 外层超时，支持通过 `POD=` 指定 worker Pod，并在 `docs/DATASYSTEM_WORKER_POOLING.md` 中补充用法。验证：`bash -n scripts/debug_datasystem_pool_smoke.sh` 与 `git diff --check` 通过。** |
