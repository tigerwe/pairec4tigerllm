...
> 最后更新: 2026-06-23 | 当前状态: F14 PaiRec Go brpc client 端到端验证通过：PaiRec `/api/recommend` 返回 `code=200/size=10`，C++ brpc inference 日志确认 `method=Recommend user=6312 code=200 items=10 latency_ms=228 backend=trtllm_cpp`，链路为 PaiRec Go -> brpc/TCP -> C++ TRT-LLM；遗留 CoreDNS 异常，当前用 `10.96.15.101:18100` ClusterIP 绕过 DNS | 下一步: 修复 CoreDNS 后将 `brpc_endpoint` 切回 Service DNS，并固化 master 构建、worker1 导入 PaiRec 小镜像流程
