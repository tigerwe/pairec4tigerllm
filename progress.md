> 2026-08-17 F14 KVC完整验收的round-3 prime假失败已收敛：远端`disabled/contention/round-3/prime-attempt-1`实际完成`195/195` Go BRPC请求和`195/195` inference服务事件，全部`code=200/items=1`，且采集到577次offload、88次onboard；日志无ERROR、Traceback、code=299、timeout或connection failure，因此不是模型、BRPC或DataSystem业务失败。外层原先只依据benchmark包装脚本进程码决定是否重跑195请求，现增加严格语义兜底：仅当`probe.exit_code=0`、summary JSON可解析且client/server事件都精确等于`PRIME_REQUESTS`时，才把非零包装退出认定为postprocess/cleanup假失败，并保存原码到`prime-attempt-N.postprocess_exit_code`；任何事件缺失或probe失败仍保持原重试/失败行为。Shell、内嵌Python和diff检查通过。下一步master拉取后重跑`REPEATS=3 bash scripts/validate_f14_kvc_burst_proxy.sh`，完成disabled/c1/c10/c100正式验收。

> 2026-08-14 F14 KVC Wrapper V2 c1远端smoke正式通过：`validate_f14_kvc_burst_proxy.sh`完成disabled+c1各1轮，真实replay `status=PASS valid=1/1`，E2E/server/BRPC/KVC=`153.151/125/4/32.048ms`，同一request_id精确`3 Set + 2 Get`、offload/onboard=`3/2`，c1 barrier wait=`0ms`、business Get=`6.708ms`、pressure Get=`0`，disabled Proxy p99=`0us`；输出`F14_KVC_BURST_PROXY_VALIDATION_OK`。结束后的幂等清理rollout成功并输出`F14_KVC_BURST_OVERLAY_RESTORE_OK`，原Inference Deployment已恢复。该证据完成c1控制面和真实请求闭环，但c10/c100压力客户端、Get重叠和成功率尚未远端验收，F14保持pending。下一步运行`REPEATS=3`完整1/10/100验收。

> 2026-08-14 F14 KVC restore陈旧备份问题修复：c1启动成功后按建议restore，生成Pod仍包含`kvc-burst-wrapper`并CrashLoop，证明默认backup已在早期排障中记录了带F14 overlay的Deployment，不能作为干净基线；原`replace --force`还导致新旧Pod短时争抢GPU/hostPort，出现FailedScheduling。restore现改为幂等现场清理，不再信任backup内容：从当前Deployment移除sidecar、主容器全部`KVC_BURST_*` env、`kvc-burst-control`挂载/volume及F14注解，使用JSON merge patch触发普通rollout，并严格确认新Pod不含sidecar；backup仅作生命周期标记后删除。Shell/内嵌Python编译及模拟陈旧overlay清理夹具通过。下一步master拉取后重新restore，确认Pod仅1个brpc-inference容器，再运行c1 smoke。

> 2026-08-14 F14 KVC c1 ready marker ENOSPC根因确认并修复：原子发布版本远端previous日志明确输出`write ready file: No space left on device`、exit=1。节点`/tmp`仍有347GiB，与故障无关；Pod共享`emptyDir.medium=Memory`的`sizeLimit=64Ki`，而该tmpfs块大小为64Ki，2432字节control文件已占一个完整block，ready临时文件首次写入即ENOSPC。overlay将共享控制tmpfs提升为`1Mi`，足够control、原子ready临时文件及最终marker；无需再次编译worker1 runtime，只需master拉取后重新apply。诊断脚本新增`KVC_BURST_CONTROL_TMPFS_TOO_SMALL`精确分类。

> 2026-08-14 F14 KVC c1 ready marker故障精确闭合并修复：远端新sidecar SHA已包含c1修复，control header=`concurrency=1/pressure_lanes=0/timeout=5/state=Ready`，PID1为sleeping且`wchan=hrtimer_nanosleep`、restart=0，证明sidecar主循环健康；唯一异常是`ready`文件已关闭但保持0字节，原`std::ofstream`未检查open/write/close状态，探针因此持续失败。marker现改为低层原子发布：同目录临时文件`open`、完整`write`、`fsync`、`close`后`rename`，任一步失败均打印errno、将共享状态置Failed并退出，不再留下0字节假ready；marker内容改用string拼接，避开文件iostream路径。共享协议与Proxy测试、原子发布结构门禁和diff检查通过。下一步worker1以`BUILD_TRTLLM=0`增量发布sidecar，master重新apply c1并确认2/2 Ready。

> 2026-08-14 F14 KVC c1第二轮启动问题修复：修复preload后sidecar不再立即`substr(4)`崩溃，但Events显示未申请GPU的sidecar无法加载主容器NVML文件（`file too short`），且c1配置虽为0条压力lane仍无条件初始化DataSystem客户端，导致ready文件长期不生成并被startup probe重启。overlay现从sidecar preload中排除`libnvidia-ml.so`，同时严格保留并校验`block_ds_consumer.so`、`stub_gpu.so`和`libabseil_dll.so`；C++ sidecar在`pressureLanes=0`时只初始化共享控制面，不连接DataSystem、不预填充，符合c1基线语义，c10/c100行为不变。新增启动阶段异常边界，后续错误会输出`kvc_burst_startup_failed`及具体stage。worker1构建入口支持`BUILD_TRTLLM=0`，只需增量重编gateway/sidecar并复用现有Release TRT库。验证：共享协议与Proxy测试通过、Shell/内嵌Python/C++结构和diff检查通过。下一步worker1增量构建，master重新apply c1。

> 2026-08-14 F14 KVC sidecar首次CrashLoop根因确认并修复：`kvc-burst-wrapper --previous`显示未捕获`std::out_of_range: basic_string::substr pos 4 > size 0`、exit 133。该特征与7月1日已知DataSystem ARM/NVIDIA runtime问题完全一致：SDK初始化会解析空GPU标识，必须使用推理容器已有的`block_ds_consumer.so + stub_gpu.so + libnvidia-ml + libabseil_dll` preload链；新overlay误将sidecar `LD_PRELOAD`清空，导致主推理容器正常而sidecar崩溃。部署脚本现从当前inference容器复制`LD_PRELOAD`、`LD_LIBRARY_PATH`、`HOST_IP`、`NVIDIA_DRIVER_CAPABILITIES`及全部`DATASYSTEM_*`环境，preload为空时在patch前失败；无需重编TRT/sidecar，只需master拉取后重新apply。诊断脚本同步将该异常分类为`KVC_BURST_DATASYSTEM_GPU_RUNTIME_PRELOAD_FAILURE`。下一步恢复Deployment后重新apply c1并确认Pod 2/2 Ready，再执行c1/c10/c100验收。

> 2026-08-14 F14 KVC Wrapper V2远端首次部署进入CrashLoop诊断：worker1已成功构建并发布带进程内Proxy的Release TRT库、gateway与`kvc_burst_wrapper` sidecar；master应用overlay后Pod为`1/2 CrashLoopBackOff`，Events显示sidecar反复重启，但尚无退出码/previous日志，不能把探针失败当作根因。新增只读一键脚本`diagnose_f14_kvc_burst_crashloop.sh`，自动采集双容器state/lastState、current/previous日志、Events、命令参数/环境/资源/挂载、sidecar二进制`ldd`、共享tmpfs和DataSystem TCP可达性，并分类OOM、动态库、参数、预填充、网络和共享控制失败；本地Shell、3段内嵌Python及diff检查通过。下一步master运行该脚本，以输出分类修复sidecar后再执行c1 smoke及c1/c10/c100正式验收。

> 2026-08-14 F14 KVC Wrapper V2进程内代理本地实现完成，待worker1/master远端验收：真实TensorRT-LLM DataSystem单Get、MGet和parallel Get调用点接入`KvcOperationProxy`，通过F19 thread-local correlation精确取得PaiRec request_id；每个request_id只允许第一次Get claim共享generation，busy/重复请求立即旁路，成功claim的业务Get才等待process-shared futex Barrier。sidecar维持最多99个exclusive DataSystem客户端和已验证的3670016字节压力Key，c1/c10/c100形成1路真实Get+C-1路压力Get；业务完成只写共享控制块并立即继续TRT，sidecar异步输出唯一`kvc_burst_complete`。实际E2E和减去`barrier_wait`的adjusted E2E同时报告，但实际值仍是权威门禁。新增幂等TRT补丁器、worker1 Release构建入口、同Pod tmpfs/sidecar可逆overlay和`reset + prime:195 + replay`验收脚本；失败/退出会恢复原Deployment。硬门禁包含disabled Proxy p99<=100us、同UUID精确3 Set+2 Get、每请求单trigger、Barrier无超时、压力全成功、>=95%业务重叠、无restart/OOM。C++ shared/proxy测试通过（含barrier期间并发请求立即busy旁路），TRT补丁测试7/7、全量Python测试80/80（skip1）、Shell及内嵌Python语法和diff check通过。本机无DataSystem/TRT GPU运行环境，下一步worker1执行`bash scripts/build_f14_kvc_burst_runtime_worker1.sh`，随后master执行`REPEATS=3 bash scripts/validate_f14_kvc_burst_proxy.sh`；只有输出`F14_KVC_BURST_PROXY_VALIDATION_OK`才算远端接通。

> 2026-08-13 F14 BRPC Wrapper 10000池/1000活跃/N-CPU shard 本地实现完成，待master远端验收：新增`brpc_burst_pool_size`、`brpc_burst_active_connections`、`brpc_burst_cpu_shards`，空CPU数组自动读取容器允许cpuset，显式数组支持任意N个唯一CPU ID，不再假设CPU1/CPU2。启动严格预连接10000条，按N个shard均匀归属；每请求从全局环连续轮换选择1000条，10轮严格覆盖全部10000条，单请求各shard连接/请求差最多1。10000个session worker常驻阻塞job channel，每请求只唤醒1000个，不再临时创建1000 goroutine，也不再逐请求并发预检全部连接。ready/start/complete事件新增pool/active/cpu_shards/shard_connections/shard_selected/shard_requests/shard_success/shard_bytes，压测门禁要求10000全连接、1000全活跃、各shard均有流量、请求差<=1、字节差<=200KB。Go race测试、services/config+recall测试、services全构建、Shell及内嵌Python静态检查通过。严格失败关闭保持不变，任何rerank failed立即结束。当前单Go进程保证逻辑shard流量均衡，物理CPU执行范围由Pod cpuset保证；不虚构goroutine逐核绑定，若必须每shard物理核硬隔离，后续采用多进程/多Pod shard。下一步master拉取后先`PHASE=smoke`验证10000连接环境能力，再stability比较新池化路径p99；暂不直接跑formal 1000样本。

> 2026-08-13 F14 BRPC Wrapper c1000 stability 首轮得到有效失败证据：100 个样本均完成1000预连接、1 Recommend+999 Health和同UUID DataSystem闭合，但Wrapper CFS门禁失败，`nr_throttled/nr_periods=21/209=10.048%`；同时c1000 E2E p99=`249.802ms`、front BRPC p99=`84.850ms`、start skew p99=`273.949ms`，不能判定压力通过。压测环境修复为PaiRec和Wrapper均保留`requests.cpu=8`调度保障但移除`limits.cpu` CFS配额；压力入口会在采样前主动修正并严格核验Wrapper资源，原性能门禁保持不变，不通过提高阈值掩盖问题。下一步重跑stability；若无throttling后front BRPC p99仍超过10ms，则根因属于现有1000 goroutine统一放行/调度架构，直接作为10000池+1000选择+CPU分片实现的基线缺陷处理。

> 2026-08-13 F14 BRPC Wrapper 全链路 c1000 压测闭环已实现，待 master 远端执行：`scripts/deploy_and_validate_pairec_brpc_wrapper_full.sh` 现同时支持 c1/c1000，c1000 每个业务样本严格要求预连接/armed=1000、1 个 Recommend、999/999 个 100KB Health 成功、burst_valid=true、业务 UUID 贯穿 Wrapper/TRT/DataSystem、Pod UID/restart 不变。新增 `scripts/run_pairec_brpc_wrapper_pressure.sh`，按 smoke=3、stability=100、formal=1000 分阶段顺序跑 c1 基线和 c1000 压力，输出 E2E/front BRPC/Wrapper/runner/burst 的 p50/p95/p99、max_active/start_skew 诊断和吞吐；硬门禁为 runner p99 增量<=10ms、c1000 front BRPC p99<=10ms，CPU period 足够时 PaiRec/Wrapper/inference throttled period<=5%。隔离 PaiRec 调整为 requests=limits=8 CPU，避免 CFS 配额污染。Shell、8 段内嵌 Python、JSON 和 diff check 已通过；尚无远端 c1000 全链路数据，下一步在 master 先执行 `PHASE=smoke bash scripts/run_pairec_brpc_wrapper_pressure.sh`，通过后依次 stability/formal；三阶段通过后才开始 10000 空闲预连接、每请求选择1000及CPU分片实现。

> 2026-08-13 F14 BRPC Wrapper 当前完整推荐链路 c1 远端验收通过：隔离 `pairec-brpc-observed-wrapper` rollout成功，3/3请求通过严格同UUID观测门禁并输出`PAIREC_BRPC_WRAPPER_FULL_C1_OK`。首请求E2E=`3675.416ms`，但同请求Wrapper/runner仅=`117/116.980ms`，约3.56秒发生在PaiRec本地进入BRPC前，符合首次用户特征文件懒加载；随后稳态E2E=`112.864/110.640ms`，front BRPC=`2.485/0.720ms`，Wrapper=`98/96ms`，DataSystem均为`Get=0/Set=2`。这证明既有Wrapper已接入生成式+向量并行召回、DeepFM和来源重排完整链路，Wrapper未破坏F19同请求归因。脚本现默认增加1次严格预热但排除其时延和日志统计，避免首次懒加载污染正式均值。下一步进入10000空闲预连接、每请求选择1000连接和CPU亲和分片设计，随后接入KVC Wrapper。
>
> 2026-08-13 F14 pause自愈兼容性修复：master首次运行新脚本在sandbox预检阶段返回`ctr: unrecognized image format`。脚本不再依赖该现场ctr可能不兼容的`images list -q`，改用已验证的普通`images list`第一列精确匹配tag；导入按`/home/zcx/pause-aarch64-3.8.tar`、`/home/zcx/master-runtime-images.tar`顺序尝试。某个archive无法被ctr直接识别时，自动执行`docker load`并以`docker save | ctr images import -`重新封装；每次导入后都复查精确tag，完整诊断写入输出目录`pause-image-import.log`。
>
> 2026-08-13 F14 BRPC Wrapper 部署脚本补齐 CRI sandbox 镜像自愈：master 多次 rollout 前 `k8s.io` containerd 缺少 `docker.io/library/pause-aarch64:3.8`，kubelet 因外网异常进入 `FailedCreatePodSandBox`。一键脚本现于任何 apply/restart 前精确检查该 tag；缺失时从默认 `/home/zcx/pause-aarch64-3.8.tar` 导入并复查，tar 缺失或导入后 tag 不匹配则提前失败，不再等待10分钟 rollout超时。root直接使用ctr，非root使用`sudo -n ctr`；PaiRec镜像导入也统一走同一k8s.io helper。可用`PAUSE_ARCHIVE`覆盖tar路径，只有显式`ENSURE_PAUSE_IMAGE=0`才跳过。
>
> 2026-08-13 F14 BRPC Wrapper 完整推荐链路接入已完成本地实现，待 master 远端验收：保留已验证的 `pairec-brpc-observed` 基线，新增隔离 `pairec-brpc-observed-wrapper` Deployment/ClusterIP；生成式召回固定经过 `192.168.100.11:18103` Wrapper 再到 worker1 TRT-LLM，向量召回与 DeepFM 继续使用 BRPC，随后执行来源配额重排。第一阶段固定 `burst_concurrency=1`、严格预连接、无 HTTP fallback、零重试和失败关闭，不产生 Health 压力流量。新增一键脚本严格校验最终响应来源、同一 request_id 的三类 PaiRec burst 事件、Wrapper Recommend trace、pipeline trace、DeepFM、rerank、TRT executor 和 native DataSystem completion，并要求 Pod UID/restart 不变。已通过 Shell、6段内嵌 Python、JSON、YAML、Go config/recall 单测和 diff check；尚未在远端执行，因此当前只能标记实现完成，不能标记链路验收完成。下一步在 master 执行 `REQUESTS=3 bash scripts/deploy_and_validate_pairec_brpc_wrapper_full.sh`，以 `PAIREC_BRPC_WRAPPER_FULL_C1_OK` 作为 c1 接通证据；通过后再设计10000空闲连接、每请求选1000连接和CPU亲和分片。
>
> 2026-08-12 F19 正式完成：最终Event审计显示唯一相关`BackOff/ImagePullBackOff`的`lastTimestamp=2026-08-12T08:01:43Z`，即北京时间16:01:43，早于本轮输出目录`20260812-162523`和约148.94秒正式workload开始时间约23分钟，确认为Pod创建阶段历史镜像拉取事件，本轮没有新增健康故障。结合1000/1000成功请求、1000/1000同UUID DataSystem completion、runner avg/p99=`95.085/105.199ms`、E2E avg/p99=`110.002/120.946ms`、五容器CPU throttling=0、固定32 output token以及Release三模式交叉验证，F19全部验收条件满足，状态改为done。后续不再重复F19性能/归因验收；Pod Event门禁已改为workload区间增量语义。
>
> 2026-08-12 F19 Release正式1000请求核心验收通过，末尾仅被历史Pod Event误报阻断最终marker：成功请求`1000/1000`、补发0、吞吐`6.714rps`；pipeline `valid=1000/1000`、missing/invalid=0，native DataSystem completion=`1000/1000`。E2E avg/p99=`110.002/120.946ms`，generative avg/p99=`99.066/109.170ms`，runner avg/p99=`95.085/105.199ms`且32 output token固定，确认Release性能恢复；native lifecycle avg/p99=`95.023/105.146ms`、闭合误差p99=`0.020us`，runner-native差p99=`0.081ms`。归因lookup/record均近0，CPU throttling五个容器均为0。最终失败来自旧门禁扫描Pod全生命周期Events，只要历史`BackOff/Unhealthy`存在即误报；现改为workload前后记录固定Pod与Event累计数，只拒绝测试期间新增事件，同时继续要求Pod不替换、restart=0。下一步master拉取后无需重跑1000请求，先审计失败Pod Event时间；若无workload期间新增事件，本轮可作为F19完成证据。
>
> 2026-08-12 F19 正式1000请求验收的随机业务失败已精确定位并隔离：失败UUID `fa4e7a98-a69e-4fd1-abf0-4f223905ed0b` 的native inference在`92.218ms`内正常完成，唯一同UUID DataSystem completion为`Set=2/10.750ms`、Get=0、failed/pending/unknown=0、`attribution_complete=true`；实际失败是生成结果无法映射出足量商品，gateway返回`code=299/items=0/items size not enough`，PaiRec多路召回降级为50个向量候选，DeepFM成功后F20来源配额重排按设计失败关闭。验收驱动新增默认0的`MAX_EXPECTED_RERANK_FAILURES`：仅显式开启时、仅对来源重排链路、仅对`code=500/msg=rerank failed`记录并跳过，继续补发直至获得目标成功样本；超过上限或任意其他错误仍立即失败。跳过UUID独立写入`expected-rerank-failures.tsv`，尝试数写入`request-attempts.json`，不混入成功样本时延和UUID归因统计；墙钟吞吐仍包含补发开销。下一步master以`MAX_EXPECTED_RERANK_FAILURES=20`、`FORCE_PAIREC_RESTART=0`重跑1000成功请求正式验收。
>
> 2026-08-12 F19 非Release时延回归已修复并完成受控验收：worker1 Release runtime构建成功，gateway/TRT SHA=`617a2172.../41353eb7...`，TRT库437MiB；三模式暖态对照为历史pair=`32 token/92.0ms=2.875ms/token`、历史gateway+当前Release TRT=`32 token/92.0ms=2.875ms/token`、当前F19 pair=`31 token/90.65ms=2.924ms/token`。当前相对历史runner仅`-1.35ms`、per-token仅`+0.049ms`，此前约85-95ms回归完全消失；唯一变量交叉配对证明根因是空`CMAKE_BUILD_TYPE`产生的未优化TRT库，不是gateway、token数、DataSystem I/O或request-id归因。当前请求归因lookup/record降至微秒内，2 Set约15.277ms仍按同一request_id闭合。1000请求验收被master的CRI sandbox镜像联网检查阻断，pause/PaiRec镜像导入后健康Pod可创建，但脚本无条件restart又销毁该Pod。现新增`FORCE_PAIREC_RESTART=0` workload-only恢复入口，避免重复制造sandbox；下一步以该开关完成正式验收，通过后F19可标记done。
>
...
> 2026-08-13 F14 runner干扰三组远端定位完成：三组均在采样前预建并硬校验10000条连接。`idle_pool` runner p99=`105.126ms`；`rpc_only`为1000活跃但0 payload，runner p99=`102.245ms`，相对基线`-2.881ms`，证明10000连接驻留和1000路空RPC调度没有造成runner回归；`payload_100k`为1000活跃且999路各100KB，runner p99=`122.676ms`，相对rpc-only增加`20.431ms`、相对基线增加`17.550ms`，超过10ms门禁。代码核对确认Health在worker1的BRPC Wrapper本地终止、不转发TRT后端，但C++ BRPC仍需接收并反序列化约100MB protobuf payload；同时Go侧每路现场分配100KB、protobuf marshal并构造frame。根因已收敛为100KB payload的数据生成/复制/网络接收/Wrapper反序列化造成的资源竞争，其中对worker1 TRT runner的直接干扰重点在同节点Wrapper的网络与protobuf处理，而非连接建立或后端Health调用。下一步在不降低流量规模的前提下做资源隔离/传输路径优化对照，优先验证Wrapper与TRT CPU/cpuset隔离，其次评估预编码和BRPC attachment，目标runner p99增量<=10ms且理想为0。
> 2026-08-13 F14 Wrapper/TRT进程级CPU隔离诊断脚本完成，待worker1远端数据：新增`diagnose_pairec_brpc_wrapper_cpu_isolation.sh`，要求Wrapper和TRT同节点、读取两者当前affinity，默认自动分配互斥的TRT 8核与Wrapper 32核（也支持显式cpuset），通过`taskset -a`约束PID1及全部现有线程并逐线程复核。脚本不重建Wrapper/TRT Pod，保留GPU runtime与DataSystem状态；隔离后复用三组10000预连接诊断，结束或异常时EXIT trap恢复原affinity，并可读取上一轮summary直接计算runner p99改善。该实验隔离用户态CPU执行，尚不隔离NIC softirq/RPS；若runner增量仍>10ms，再进入IRQ/RPS/NUMA观测。Shell、2段内嵌Python和diff检查通过。下一步master执行时传入上一轮`20260813-155747-n100/summary.json`，验收隔离后runner增量。
> 2026-08-13 F14 CPU隔离首跑在idle_pool正式第5条被严格rerank失败关闭中断：CPU隔离成功（TRT `0-7`、Wrapper `8-39`）、10000连接预建成功、预热成功、前4条成功；第5条UUID `2383b40a-924c-4643-b3bc-b17c8e1ace7a`返回`code=500/msg=rerank failed`，与隔离和BRPC无关。根因是诊断链未透传用户而回落`USER_ID=1`，native TRT又按递增seed采样，无法靠PaiRec temperature固定结果。现诊断默认并透传历史稳定用户`6312`，每组在10000连接确认后、计时前执行10次完整链路连续资格检查；任何资格或正式请求失败仍立即终止，不跳过、不补样、不放宽重排。affinity恢复逐线程噪声改写独立日志，避免覆盖真正错误。Shell、12段内嵌Python和diff检查通过。下一步重跑同一CPU隔离诊断。
> 2026-08-13 F14 CPU隔离第二次远端运行仍被严格rerank随机失败中断：`idle_pool`已完成100条，runner p99=`105.106ms`；`rpc_only`完成资格检查及11条正式请求后，第12条UUID `58668458-0299-4655-a0a9-656cea3f98d7`返回rerank failed。稳定用户和10次资格检查不能约束native TRT每请求递增seed，因此不能保证后续100条。诊断编排现改为先备份Inference Deployment参数，临时把`--trt_top_k`设为`1`消除seed对采样结果的影响，在完全相同runtime/生成参数下先跑未隔离三组，再将TRT与Wrapper全部线程绑定互斥cpuset跑隔离三组，并直接生成`comparison.json`；业务失败仍立即终止。EXIT trap恢复CPU affinity和原始TRT参数并等待滚动完成，恢复失败会令脚本失败且保留独立日志。该改动只影响诊断期间采样策略，不修改推荐、rerank fail-close或Wrapper实现。Shell、4段内嵌Python和diff检查通过；下一步master拉取后执行100样本同配置对照。
> 2026-08-13 F14 确定性CPU隔离对照完成：两组均使用临时`trt_top_k=1`并完成每组三臂各100条。未隔离时payload_100k相对idle_pool的runner p99增量=`23.739ms`；TRT绑定`0-7`、Wrapper绑定`8-39`后增量=`24.006ms`，改善=`-0.267ms`，门禁失败。隔离组idle_pool/rpc_only/payload_100k runner p99分别为`103.946/101.062/127.952ms`，纯1000路空RPC增量=`-2.884ms`，100KB payload附加增量=`26.890ms`；三项CPU throttling均为0。由此排除10000空闲连接、1000路空RPC调度、用户态CPU核竞争和CFS throttling，问题进一步收敛到同节点约100MB payload处理引发的NIC RX softirq/RPS、内存带宽/LLC或NUMA竞争。脚本已进入CPU affinity与原始TRT采样参数恢复阶段，但当前回传未包含恢复成功状态，需先确认`trt_top_k=50`和Inference rollout完成。下一步将Wrapper移到master、TRT继续在worker1，以完全相同10000预连接/1000活跃/999x100KB/100样本做跨节点对照；若runner增量降至<=10ms，则确认同节点资源竞争，否则继续采集IRQ/RPS/NUMA/内存带宽证据。
> 2026-08-13 F14 Wrapper跨节点对照已完成本地实现，待master远端执行：新增独立`brpc-burst-wrapper-master`，通过master hostNetwork监听`192.168.100.12:18104`，仅真实Recommend跨25G转发到worker1 `192.168.100.11:18100`，999路100KB Health在master本地终止；现有worker1 `18103` Wrapper保留为同节点基线。新增一键脚本先确保master/worker1 pause镜像与master runtime、必要时从本地镜像提取单文件Wrapper binary，硬校验两个Wrapper二进制SHA一致及实际节点拓扑，再临时设置`trt_top_k=1`，依次运行同节点和跨节点各三臂100样本，输出runner p99增量与改善。异常或成功退出均恢复原TRT采样参数，并删除本次新建的master诊断Deployment；若同名Deployment事先存在则拒绝覆盖。原严格rerank fail-close、10000预连接和业务/日志/Pod稳定性门禁保持。Shell、3段内嵌Python、YAML契约和diff检查通过，尚无远端时延数据。
> 2026-08-13 F14 跨节点首轮预检被二进制一致性门禁正确阻断：master Wrapper已在`18104`启动且Recommend转发成功，拓扑确认`inference=worker1/colocated=worker1/cross_node=master`，但worker1在用的hostPath Wrapper SHA为`0e7bc7fd...`，master镜像提取版本为`aa95a674...`，不能作为有效节点对照；临时Deployment已自动删除，未进入计时。修正为master使用专用hostPath `/home/zcx/bin/brpc_burst_wrapper_master_diag`，每次部署前通过25G SSH/SCP直接同步worker1当前`/home/zcx/bin/brpc_burst_wrapper`，源端、传输文件、激活文件和两个Pod内SHA四重一致后才运行，不覆盖其他master二进制。
> 2026-08-13 F14 Wrapper跨节点100样本对照通过：同节点worker1 Wrapper的payload_100k相对idle_pool runner p99增量=`27.967ms`，迁移Wrapper到master后增量降至`2.577ms`，改善=`25.390ms`，通过`<=10ms`硬门禁。跨节点三臂runner p99分别为idle_pool `100.275ms`、rpc_only `94.702ms`、payload_100k `102.852ms`；纯空RPC增量=`-5.572ms`，100KB payload附加增量=`8.150ms`，CPU throttling均为0。100/100业务请求通过，10000连接预建、1000活跃和999路100KB压力保持。该结果确认主要干扰来自约100MB payload在GPU节点同机接收/反序列化/内存路径造成的共享资源竞争；把Health压力终止点移到master可消除绝大部分runner回归。front BRPC p99=`64.616ms`仅作观测，不是门禁。当前输出显示已进入原始TRT采样配置恢复，仍需确认`trt_top_k=50`、Inference rollout完成及临时master Wrapper已删除。下一步做跨节点正式1000样本c1与c1000 payload验收，不再重复无结论的同节点1000样本；runner p99增量继续要求`<=10ms`。
> 2026-08-13 F14 同节点Wrapper payload路径优化已实现，待master远端100样本A/B：新增可选`brpc_burst_payload_transport=protobuf|attachment`，默认`protobuf`保持旧行为。attachment模式只分配一份共享100KB只读缓冲，通过bRPC attachment和Go `net.Buffers.WriteTo`散布写复用，不再由999路分别构造100KB protobuf字段和完整连续frame；Wrapper同时统计protobuf/attachment字节并通过backend返回实际transport，压力协调器对非零payload执行硬校验，避免配置未生效的假A/B。新增一键`diagnose_pairec_brpc_wrapper_attachment.sh`，在master构建并同步PaiRec与Wrapper、自动补齐worker1 pause镜像、临时设置确定性`trt_top_k=1`，依次执行同节点protobuf和attachment各三臂100样本，异常或成功均恢复原TRT参数。验收保持10000预连接、1000活跃、999x100KB、严格rerank和runner p99总增量`<=10ms`；本地Go race、bRPC attachment线协议、Shell、JSON和12段内嵌Python验证通过，尚无远端时延结论。
> 2026-08-13 F14 attachment A/B远端首跑未形成时延结论：attachment的idle_pool与rpc_only完成，payload_100k确认10000连接预建、1000活跃、transport=attachment、10次资格检查和预热均成功，但正式第6条请求`7aa3a576-d615-400b-be55-b7cc138c1cea`返回`code=500/msg=rerank failed`，脚本按严格fail-close立即终止并进入TRT参数恢复。新增只读`diagnose_pairec_wrapper_rerank_failure.sh`，支持按REQUEST_ID或失败轮次目录自动定位非200响应，采集PaiRec/Wrapper/Inference当前及previous容器日志、Deployment rollout和`trt_top_k`，分类推理items不足、generative BRPC失败、零生成候选、source quota fail-close或pressure失败；若恢复rollout已替换原Inference Pod，会明确报告原生完成事件丢失而不误判。Shell、2段内嵌Python和diff检查通过。下一步先对该UUID运行诊断，再决定修复生成候选稳定性或直接重跑A/B。
> 2026-08-13 F14 首个失败UUID事后诊断结果：PaiRec generative span明确为`brpc Recommend failed ... read brpc header failed: EOF`，随后`generative_input=0`，rerank按`minimum_generative=1`正确fail-close；故障不是重排逻辑自身，也尚不能归因于attachment编码。当前`trt_top_k=50`说明外层恢复成功，但恢复rollout已替换原Inference Pod，Wrapper和Inference均无该UUID日志，无法区分Wrapper转发过程中断连、Wrapper进程异常或Inference连接关闭。A/B编排已改为内层失败后、外层恢复TRT前自动运行诊断并冻结三侧current/previous日志、Pod YAML/describe、UID/restart与events；失败目录改为递归搜索。Wrapper新增`recommend_start`与带最终code/error的完成日志，以区分请求未到达、到达后中断和后端显式失败。下一步重建Wrapper/PaiRec后重跑100样本，若再次EOF将保留可定责现场。
> 2026-08-13 F14 第二次attachment运行再次出现EOF，冻结现场已把责任边界收敛到Wrapper响应路径：UUID `da4d6fea-2ac0-412b-829c-c5e57930e337`存在Wrapper `recommend_start`；Inference同UUID完整输出DataSystem completion、executor completion和`code=200/items=3/latency=111ms`；Wrapper没有最终Recommend日志；PaiRec读取前端BRPC header时EOF，随后生成候选归零、rerank正常fail-close。由此排除Inference推理失败、候选不足和rerank自身问题，故障发生在Wrapper收到后端成功响应后到前端响应完成前。诊断脚本新增`WRAPPER_RESPONSE_PATH_INTERRUPTED`分类，并直接汇总Wrapper/Inference container restart、current/lastState和termination reason。下一步从本次冻结目录的pods.json、describe、events和previous wrapper log确认OOMKilled/信号/探针重启；在确认退出原因前不继续做时延A/B。
> 2026-08-14 F14 BRPC Wrapper attachment实验已回退，runner同节点劣化转为已知遗留项：在Wrapper内存限制临时提高到8Gi后，100请求全部成功，排除了此前2Gi限制触发`OOMKilled -> PaiRec EOF -> generative=0 -> rerank fail-closed`对性能样本的污染。相同10000预连接/1000活跃/每路100KB条件下，protobuf的runner p99总增量=`23.729ms`，attachment虽将start skew p99降至`5.608ms`、front BRPC p99降至`14.523ms`，但runner p99总增量扩大为`31.911ms`，未满足`<=10ms`门禁。按当前决策恢复protobuf-only压力路径，删除attachment配置、发送实现和专属A/B脚本，保留通用EOF/OOM现场诊断；同GPU节点大payload造成的NIC/内核网络栈/内存路径竞争暂不继续优化，不阻塞Wrapper现有功能收口。
