...
> 最后更新: 2026-06-12 | 当前状态: 正式 inference runtime 镜像已复验通过，K8s PaiRec -> inference -> TRT HTTP 基线闭环稳定；历史跑通容器 `3d25ebe028d6` 已证明同一 `qwen3_rec_v4` engine 可初始化 TensorRT-LLM C++ DataSystem，且实际使用的 `/TensorRT-LLM` 是宿主机 bind mount；K8s 当前失败优先收敛为正式镜像未包含相同 C++ runtime/LD_PRELOAD helper | 下一步: 用 `scripts/build_datasystem_runtime_base_image.sh` 将挂载的 patched `/TensorRT-LLM` 复制进 `zcx-pairec-ds-runtime:v1`，再重建 `pairec-inference:k8s-arm64-ds-runtime-v1` 并验证 C++ DataSystem TRACE
