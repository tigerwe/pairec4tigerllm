...
> 最后更新: 2026-06-12 | 当前状态: 正式 inference runtime 镜像已复验通过，K8s PaiRec -> inference -> TRT HTTP 基线闭环稳定；K8s 当前 `qwen3_rec_v4` 与历史跑通容器的 engine config 一致，C++ DataSystem 验证差异优先怀疑 TRT-LLM C++ runtime patch/动态库 | 下一步: 在 inference Pod 内检查 DataSystem TRACE 字符串和 C++ patch 是否实际进入镜像；必要时再构建独立 `qwen3_rec_v4_paged_fmha` engine 做后备验证
