> 2026-08-10 F20 DeepFM 后来源配额重排本地实现完成，待远端验收：新增进程内 `source_quota_tail`，对 DeepFM 排序后的50候选按请求size输出，保留 `min(2, generative_input, size)` 个生成式候选并连续置于末尾，两个来源内部保持DeepFM顺序且不修改分数。缺少生成式、未知来源、重复商品、候选/输出数量不满足契约时fail-closed，Controller返回`code=500/msg=rerank failed/items=[]`。隔离`pairec-brpc-observed`配置已启用；请求级rerank span、低基数Prometheus、响应来源顺序、配额完整性、rerank p99<=1ms和E2E p99<=122.622ms门禁已接入。Go race（rerank/observability/sort/web）、完整services build、Python summary unittest 6/6、Shell/JSON/diff检查通过。F20状态保持in_progress，下一步远端执行3次smoke、1000次正式测试及缺生成式故障注入。
>
