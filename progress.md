...
> 最新补充(7/3): master 新 Pod 创建过程中连续暴露 pause sandbox 镜像缺失与 Calico `calico/cni` 镜像拉取证书问题。新增统一入口 `scripts/sync_master_runtime_images.sh`，默认从 worker1/188 的 `k8s.io` containerd namespace 同步 pause + Calico 镜像到 master/189，并可用 `EXTRA_IMAGES` 追加 nvidia-device-plugin 等运行时镜像；旧的 `scripts/sync_pause_image_to_master.sh` 和 `scripts/sync_calico_images_to_master.sh` 保留为兼容 wrapper。验证：三个脚本 `bash -n` 通过，新增脚本已设置可执行权限，`git diff --check` 通过。

## 时间线

| 日期 | 进度 |
|------|------|
| **7/3** | **合并 master 运行时基础镜像同步脚本。新增 `scripts/sync_master_runtime_images.sh`，统一处理 `pause-aarch64:3.8`、Calico DS/Deployment 镜像和可选 `EXTRA_IMAGES` 的 worker1→master 离线导出/导入；支持 `RESTART_KUBELET`、`RESTART_CONTAINERD`、`DELETE_MASTER_CALICO_PODS`、`DELETE_NON_RUNNING_PODS`。`scripts/sync_pause_image_to_master.sh` 与 `scripts/sync_calico_images_to_master.sh` 改为 wrapper，分别设置 `SYNC_PAUSE/SYNC_CALICO` 和历史 archive 默认路径后转发统一脚本。验证：`bash -n` 覆盖三个脚本，`git diff --check` 通过。** |
