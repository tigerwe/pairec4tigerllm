...
> 最后更新: 2026-08-03 | 当前状态: F14 已实现同实例 BRPC burst Wrapper，本地验证完成，待 188/189 环境校准正式 Recommend 的 30-40ms BRPC 通信差值。严格25G历史结果见 `docs/F14_STRICT_25G_PRESSURE_RESULTS_2026-08-02.md`；F15 NPU迁移记录继续保留。
> 最新补充(8/3-F14 burst Wrapper): `scripts/probe_go_brpc_client.go` 新增 `method=burst`，每轮固定 `1 Recommend + N-1 Health`，全部 lane 先 armed 后统一释放到同一推理实例；正式请求与100KB压力payload独立配置，输出 `armed_workers/max_active_workers/start_skew_us`、Health成功率、正式请求 client wall/inference/BRPC delta 和结构化JSON。新增 `scripts/benchmark_go_brpc_burst_wrapper.sh`，默认对10/25/50/100/200/400/600/800/1000并发各跑20轮，必要时自动细化跨越30-40ms目标的区间；门禁为Recommend和Health全成功、实际max active达到95%、Pod UID/restart不变且无crash marker。本地证据：Go探针build通过；同步屏障与统计单测通过；`services/recall`测试通过；shell及两个内嵌Python聚合器语法通过；失败夹具正确输出业务/压力失败并返回非零；成功聚合夹具用35/37ms选出p50=36.0ms、p95=36.9ms并生成selected.env。当前未在远端GPU/K8s执行，下一步先在真实 `192.168.100.11:18100` 用100并发×3轮快速验证，再跑完整阶梯。
