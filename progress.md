...
> 最后更新: 2026-06-12 | 当前状态: 正式 inference runtime 镜像已复验通过，K8s PaiRec -> inference -> TRT HTTP 基线闭环稳定；历史跑通容器 `3d25ebe028d6` 已证明同一 `qwen3_rec_v4` engine 可初始化 TensorRT-LLM C++ DataSystem，K8s 当前失败优先收敛为正式镜像未包含相同 C++ runtime/LD_PRELOAD helper | 下一步: 将历史容器 commit 为 `zcx-pairec-ds-runtime:v1`，基于它重建 `pairec-inference:k8s-arm64-ds-runtime-v1` 并验证 C++ DataSystem TRACE

## 时间线

| 日期 | 进度 |
|------|------|
| **6/12** | **确认 K8s C++ DataSystem 差异来自 runtime 镜像而非 engine：用户在历史跑通容器 `3d25ebe028d6` 内用同一 `qwen3_rec_v4` 启动服务，日志出现 `[TensorRT-LLM][Datasystem] Create Datasystem class`、`Init KvCache Manager DataSystem success`，且 `Blocks per window size` 显示 `primaryBlocks=32 secondaryBlocks=28`，说明该容器内 TensorRT-LLM C++ patch 与 `block_ds_consumer.so/stub_gpu.so/libabseil` 预加载链路有效。已将正式 inference runtime 构建链路改为可指定 `BASE_IMAGE`，默认基于 `docker.io/library/zcx-pairec-ds-runtime:v1`；Dockerfile 在镜像内编译 `/opt/pairec/lib/block_ds_consumer.so` 与 `/opt/pairec/lib/stub_gpu.so`；entrypoint 启动时清理旧 `LD_PRELOAD`，按顺序加载 block/stub/NVML/abseil；K8s manifest 默认 tag 改为 `docker.io/library/pairec-inference:k8s-arm64-ds-runtime-v1`，避免复用旧失败 tag。下一步在 Docker 宿主机先检查 `/home/TensorRT-LLM` 非 bind mount，再 `docker commit 3d25ebe028d6 docker.io/library/zcx-pairec-ds-runtime:v1`，重建/导入新镜像后复验 C++ offload/onboard TRACE** |
