> 2026-08-11 F19 首个真实 PaiRec UUID 精确归因 smoke 通过：请求 `08131bb7-9980-4137-bc7a-b337c0e847ac` 经 PaiRec -> BRPC -> native TRT 成功返回，inference 输出唯一同 UUID `datasystem_request_complete`，`attribution_complete=true`，pending/unknown/Get失败/Set失败均为0；首请求同时触发 `datasystem_attribution_ready`。本样本 Get/Set 均为0，准确表示该新鲜缓存请求没有发生 KVC offload/onboard，因此只证明身份传播、生命周期闭合和零操作语义，不能作为真实 DataSystem I/O 归因的最终证据。真实 I/O 验收现已封装为 `validate_f19_datasystem_real_io.sh`：默认验证严格 runtime，再逐轮 reset、prime 195请求并 replay，共3轮；每轮同时要求旧 offload/onboard 窗口证据为3/2，以及唯一同 UUID native completion 的Set/Get为3/2、complete=true、失败/pending/unknown全0。动态正反夹具确认 native 3/2时PASS、旧窗口3/2但native 0/0时FAIL；F19相关17项测试通过。下一步在master执行该一键脚本取得真实I/O证据，再跑1000请求严格 join 与开关A/B。F19保持in_progress。
>
