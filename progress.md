...
> 最后更新: 2026-06-22 | 当前状态: F14 真实 C++ TRT-LLM brpc 后端远程 K8s/GPU smoke 通过：`brpc_inference_server --backend=trtllm_cpp` 完成 TensorRT-LLM plugin preflight、engine 反序列化、DataSystem KV 初始化并监听 `0.0.0.0:18100`；brpc Health 与 Recommend 均成功，Recommend 返回 5 个 items | 下一步: 开始 PaiRec Go 侧 brpc/TCP client 接入，替换当前 HTTP 调用路径并保留回退开关
