> 2026-08-10 F20 DeepFM 后来源配额重排正向远端验收通过，待失败关闭故障注入：隔离 `pairec-brpc-observed` 正式运行1000请求，`valid=1000/1000, missing=0, invalid=0`；客户端E2E avg/p50/p95/p99/max=`110.251/109.237/117.395/120.318/127.692ms`，低于`122.622ms`门禁；rerank avg/p50/p95/p99/max=`0.015/0.013/0.023/0.030/0.589ms`，p99远低于`1ms`。保留HTTP基线100请求p99=`118.604ms`，BRPC相对增量=`1.713ms`。PaiRec CPU throttled period=`0.067%`，其余四个adapter/backend容器均为0%，无fallback，最终分类`PAIREC_PURE_BRPC_OBSERVABILITY_OK`。本地Go race、services测试、Python summary unittest 6/6及静态检查均通过。新增 `scripts/test_pairec_source_quota_rerank_fail_closed.sh`，自动备份配置、注入不足配额、校验500/error trace，并在成功、失败或信号中断时恢复配置及验证200；Shell/JSON/diff静态检查通过。F20继续保持`in_progress`，待master执行该脚本后收口。
>
