...
> 最后更新: 2026-06-22 | 当前状态: F14 真实 C++ TRT-LLM brpc 后端已进入远程 K8s/GPU 启动验证：`brpc_inference_server --backend=trtllm_cpp` 已能在 worker1 上加载到 TensorRT-LLM Executor 初始化阶段；已通过降低 `max_batch_size/max_num_tokens/max_kv_tokens/host_cache_size` 解决前序 OOMKilled，当前失败点定位为 TensorRT-LLM engine 反序列化前未注册 `Gemmtensorrt_llm` 插件 | 下一步: 重新构建/覆盖小 binary，验证显式 `initTrtLlmPlugins()` + 链接 `libnvinfer_plugin_tensorrt_llm.so` 后 Pod Ready，再跑 brpc smoke
