...
> 最后更新: 2026-06-12 | 当前状态: 正式 inference runtime 镜像已复验通过，K8s PaiRec -> inference -> TRT HTTP 基线闭环稳定；`pairec-inference:k8s-arm64-ds-runtime-v1` 已恢复 TensorRT-LLM C++ DataSystem 初始化，启动日志显示 `secondaryBlocks=28`、`Create Datasystem class` 与 `Init KvCache Manager DataSystem success` | 下一步: 运行 C++ KV pressure/replay 压测，确认 `[Datasystem][TRACE] op=offload/onboard` 并输出 Set/Get 分位数
