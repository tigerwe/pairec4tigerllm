...
> 最后更新: 2026-06-16 | 当前状态: F14 brpc phase-1 PoC 设计、代码骨架和 brpc 环境脚本已落地，采用 native brpc/`baidu_std` over TCP 的 C++ gateway sidecar，先转发到现有 Python Flask `/recommend`，不改稳定 HTTP/TRT 主链路；已固化 brpc SDK base image 构建、gateway 镜像构建、master 到 worker1 镜像分发/import、K8s apply 和 smoke 验证脚本 | 下一步: 在远程执行 SDK/gateway 构建与 worker1 导入，apply `k8s/deployment-inference-brpc-image.yaml`，执行 `scripts/test_brpc_gateway_smoke.sh` 验证 `Health`/`Recommend`
