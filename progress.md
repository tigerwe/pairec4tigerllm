> 2026-08-11 F19 V2 worker1编译流程已固化：新增`scripts/build_f19_attribution_runtime_worker1.sh`，在worker1自动选择本地`pairec-brpc-inference:k8s-arm64-trtllm-multisequence-kvc-ctx224-v1`优先镜像，同时重编`libtensorrt_llm.so`和`brpc_inference_server`。脚本固化了上次手工编译暴露的`/host-driver/libcuda.so.1`挂载以及`libcudadevrt.a/libcudart_static.a`链接搜索路径，并在覆盖`/home/zcx/pairec-f19-runtime`前强制检查V2 marker、output-token trace字段和`ldd`无缺库。先前临时编译容器的精确名称未写入项目交接记录，因此不作伪造还原；脚本会尝试扫描worker1 shell history中的相关命令，并支持`BUILD_IMAGE`显式覆盖。本地Shell/Python静态检查和8项专项测试通过；下一步在worker1运行该脚本，然后由master apply overlay并执行3请求smoke与3对1000请求A/B。F19保持in_progress。
>
