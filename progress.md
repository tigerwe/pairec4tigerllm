...
> 最后更新: 2026-06-12 | 当前状态: 正式 inference runtime 镜像已复验通过，K8s PaiRec -> inference -> TRT HTTP 基线闭环稳定；K8s 当前 `qwen3_rec_v4` engine 配置不支持 paged context FMHA，运行时会禁用 KV block reuse | 下一步: 在 worker1 构建独立 `qwen3_rec_v4_paged_fmha` engine，切换 inference DS 验证版并复验 C++ offload/onboard TRACE

## 时间线

| 日期 | 进度 |
|------|------|
| **6/12** | **定位 K8s C++ DataSystem KV 验证失败根因：远程 verifier 显示 `reuse disabled warning seen=True`、offload/onboard/TRACE 均为 0；进一步检查当前 K8s 挂载的 `/app/trt_engines/qwen3_rec_v4/config.json`，确认 `build_config/plugin_config/context_fmha=False`、`use_paged_context_fmha=False`，因此 TensorRT-LLM 在启动时按 upstream 逻辑关闭 `kv_cache_enable_block_reuse`，压力流量不会进入 C++ offload/onboard 路径。为避免覆盖已验证 HTTP/TRT 基线 engine，新增 `k8s/job-build-trt-engine-paged-fmha.yaml` 与 `scripts/k8s_build_trt_engine_paged_fmha.sh`，用于在 worker1/GPU 上构建独立 `trt_engines/qwen3_rec_v4_paged_fmha`，构建参数包含 `--kv_cache_type paged --remove_input_padding enable --context_fmha enable --use_paged_context_fmha enable`；`k8s/deployment-inference-image.yaml` 的 DS 验证版默认 `TRT_ENGINE_DIR` 已切到该新目录。当前本地 `bash -n` 与 YAML parse 通过；待远程构建 engine、apply inference 后复验日志不再出现 `KV cache reuse disabled` 且出现 `[TensorRT-LLM][Datasystem][TRACE] op=offload/onboard`** |
