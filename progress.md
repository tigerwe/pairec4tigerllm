...
| 日期 | 进度 |
|------|------|
| **7/22** | **KVC压力工具改为长驻SDK客户端。修复dsbench总key数固定、反复WarmUp/Init导致配置并发与实际压力不一致的问题；新增实际QPS/Gbps/max_active_calls证据和严格压力验收。静态检查通过，下一步在188链接SDK并跑 `kvc-get` 16并发。** |
