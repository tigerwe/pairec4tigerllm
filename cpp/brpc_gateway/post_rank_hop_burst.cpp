...
class BurstCoordinator::Impl {
 public:
  ...
  bool Init(const BurstConfig& config, std::string* error) {
    ...
    for (int begin = 0; begin < config.concurrency; begin += config.startup_batch_size) {
    ...
    *result = BurstResult{};
        ...
        std::chrono::milliseconds(config_.business_timeout_ms);
    {
      ...
      if (!round->cv.wait_until(lock, deadline, [&] {
            return round->ready == config_.concurrency;
          })) {
    ...
    std::unique_lock<std::mutex> lock(round->mutex);
    if (!round->cv.wait_until(lock, deadline, [&] { return round->business_done; })) {
      result->error = "post-rank hop2 business timed out";
      return false;
    }
  ...
  void WorkerMain(int lane) {
    ...
    while (!shutdown_.load(std::memory_order_acquire)) {
      ...
      bool ok = false;
      std::string error;
      pairec::pipeline::RankResponse rank_response;
      brpc::Controller controller;
      if (lane == 0) {
        controller.set_timeout_ms(config_.business_timeout_ms);
        worker.stub->Rank(&controller, &round->request, &rank_response, nullptr);
        ok = !controller.Failed() && rank_response.code() == 200;
      } else {
      ...
      if (controller.Failed()) error = controller.ErrorText();
      ...
      bool finish = false;
      {
        ...
        if (lane == 0) {
          round->business_done = true;
          round->business_success = ok;
          round->business_start_ns = started_ns;
          round->business_end_ns = ended_ns;
          round->business_response = rank_response;
          round->business_error = error;
        } else {
