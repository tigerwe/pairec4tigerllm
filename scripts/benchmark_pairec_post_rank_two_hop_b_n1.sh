...
outer_complete=one(pairec,"pairec_post_rank_hop1_brpc_burst_complete")
inner_complete=one(hop1,"pairec_post_rank_hop2_brpc_burst_complete")
result={
...
assert result["client_e2e_ms"] <= float(sys.argv[5]),result
assert result["post_rank_two_hop_ms"] <= float(sys.argv[6]),result
