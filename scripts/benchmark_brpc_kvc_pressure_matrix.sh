...
MATRIX_STRICT_COUNTS="${MATRIX_STRICT_COUNTS:-1}"
...
BRPC_LOAD_ENDPOINT="${BRPC_LOAD_ENDPOINT:-192.168.100.11:18101}"
BRPC_LOAD_ENDPOINT_2="${BRPC_LOAD_ENDPOINT_2:-192.168.100.11:18102}"
BRPC_SHARED_POD_SELECTOR="${BRPC_SHARED_POD_SELECTOR:-app=inference-brpc-trtllm}"
BRPC_SHARED_CONTAINER="${BRPC_SHARED_CONTAINER:-brpc-inference}"
  ...
  case "$name" in
    ...
    brpc-c100x2)
      args+=(
        MODE=brpc
        BRPC_ENDPOINT="$BRPC_ENDPOINT"
        BRPC_LOAD_ENDPOINT="${BRPC_LOAD_ENDPOINT},${BRPC_LOAD_ENDPOINT_2}"
        BRPC_LOAD_PAYLOAD_BYTES="$BRPC_PAYLOAD_BYTES"
        BRPC_LOAD_CONCURRENCY=200
        BRPC_LOAD_REQUESTS="$BRPC_REQUESTS"
        BRPC_LOAD_QPS=0
        BRPC_LOAD_REUSE_CONNECTIONS=1
      )
      ;;
    brpc-shared-c10|brpc-shared-c100)
      local concurrency="${name#brpc-shared-c}"
      args+=(
        MODE=brpc
        BRPC_ENDPOINT="$BRPC_ENDPOINT"
        BRPC_LOAD_ENDPOINT="$BRPC_ENDPOINT"
        BRPC_LOAD_POD_SELECTOR="$BRPC_SHARED_POD_SELECTOR"
        BRPC_LOAD_CONTAINER="$BRPC_SHARED_CONTAINER"
        BRPC_LOAD_PAYLOAD_BYTES="$BRPC_PAYLOAD_BYTES"
        BRPC_LOAD_CONCURRENCY="$concurrency"
        BRPC_LOAD_REQUESTS="$BRPC_REQUESTS"
        BRPC_LOAD_QPS=0
    ...
    kvc-1.75m-c100)
      args+=(
        ...
        KVC_GET_KEY_COUNT=40
        KVC_SET_KEY_COUNT=60
        KVC_BATCH_NUM=1
        KVC_THREAD_NUM=1
        KVC_GET_CLIENTS=40
        KVC_SET_CLIENTS=60
      ...
      ;;
    combined-c100-kvc100)
      args+=(
        MODE=combined
        BRPC_ENDPOINT="$BRPC_ENDPOINT"
        BRPC_LOAD_ENDPOINT="$BRPC_LOAD_ENDPOINT"
        BRPC_LOAD_PAYLOAD_BYTES="$BRPC_PAYLOAD_BYTES"
        BRPC_LOAD_CONCURRENCY=100
        BRPC_LOAD_REQUESTS="$BRPC_REQUESTS"
        BRPC_LOAD_QPS=0
        BRPC_LOAD_REUSE_CONNECTIONS=1
        KVC_LOAD_HOST="$KVC_LOAD_HOST"
