...
case "$IMPORT_CMD" in
  ctr)
    ...
    ssh "$WORKER" "ctr -n k8s.io images import '$REMOTE_TAR' && ctr -n k8s.io images ls | grep -F 'pairec-brpc-gateway'"
    ;;
  docker)
    echo "Importing on worker with docker"
    ssh "$WORKER" "docker load -i '$REMOTE_TAR' && docker images | grep -F 'pairec-brpc-gateway'"
    ;;
  none)
