...
"$KVC_UB_PROBE_BIN" \
  ...
  --prefix "$PREFIX" \
  --cleanup true | tee "$REPORT"

grep -q '^KVC_UB_INTEGRITY_PASS$' "$REPORT" || die "probe did not report integrity success"
...
[[ "$set_ub" -eq "$ITERATIONS" ]] \
  || die "expected $ITERATIONS UB Set records, observed $set_ub"
[[ "$get_ub" -eq "$ITERATIONS" ]] \
  || die "expected $ITERATIONS UB Get records, observed $get_ub"
[[ "$tcp" -eq 0 ]] || die "observed $tcp TCP fallback records"
[[ "$total" -eq $((ITERATIONS * 2)) ]] \
  || die "expected $((ITERATIONS * 2)) Set/Get access records, observed $total"

echo "KVC_UB_ACCESS_LOG_PASS prefix=$PREFIX set_ub=$set_ub get_ub=$get_ub tcp=$tcp"
